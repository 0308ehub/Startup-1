...
-- Fix 1: Ensure all existing users have collections
...
-- Fix 4: Create a function to manually ensure collection exists
...
-- Fix 5: Test the function for the current user
-- (This will create a collection if one doesn't exist)
SELECT 
    'Testing collection creation for current user' as test_name,
    ensure_user_collection(auth.uid()) as collection_id;
...
-- Show the current user's collection
SELECT 
    u.email,
    c.id as collection_id,
